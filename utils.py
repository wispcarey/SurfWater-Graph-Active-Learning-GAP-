...
if __name__ == '__main__':
    ...
    parser.add_argument('--name_index', nargs='+', type=int, default=[0,1,2,3],
                        help='the indices of river names in the list applied to train-test split')
    parser.add_argument('--train_percent', type=float, default=0.75,
                        help='the percentage of training data, must be value between 0 and 1')
    parser.add_argument('--rand_seed', type=int, default=42,
                        help='the random seed when sampling images')
    parser.add_argument('--save_path', type=str, default='data/train_test_dic.npy',
                        help='path to the directory of the train-test data')
    args = parser.parse_args()

    data = np.load(args.data_dic, allow_pickle='TRUE').item()
    river_names = np.load(args.river_names)
    names = river_names[args.name_index]

    np.random.seed(args.rand_seed)
    train_test_dic = train_test_split(data, names, args.train_percent)

    np.save(args.save_path, train_test_dic)
